...
function codegenContractFactory(contract, abi, bytecode) {
    const constructorArgs = contract.constructor && contract.constructor[0]
        ? generateInputTypes(contract.constructor[0].inputs)
        : "";
    const constructorArgNames = contract.constructor && contract.constructor[0]
...
function generateFunction(fn) {
    return `
  ${fn.name}(${generateInputTypes(fn.inputs)}${!typechain_1.isConstant(fn) && !typechain_1.isConstantFn(fn) ? "overrides?: TransactionOverrides" : ""}): Promise<${fn.stateMutability === "pure" || fn.stateMutability === "view"
